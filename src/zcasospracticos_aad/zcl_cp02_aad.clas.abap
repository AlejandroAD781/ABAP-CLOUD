...
CLASS zcl_cp02_aad IMPLEMENTATION.
...
*-----------------------------------------------------------------------
* TODOS LOS CAMPOS SON TIPOS GENERICOS
*
* Campos que se beneficiarían de un Elemento de Datos:
* - ID_RESERVA : Etiqueta descriptiva y documentación.
* - AEROLINEA  : Dominio con códigos de aerolínea y ayuda de búsqueda.
* - NUM_VUELO  : Dominio para formato del número de vuelo.
* - PASAJERO   : Elemento de datos con etiqueta "Nombre del pasajero".
* - FECHA      : Elemento de datos estándar de fecha (DATS) con etiquetas.
* - PRECIO     : Elemento de datos con descripción e importe asociado a una moneda.
* - ESTADO     : Dominio con valores fijos (A = Activa, C = Cancelada) y ayuda F4.
*-----------------------------------------------------------------------

    DATA lt_reserva TYPE STANDARD TABLE OF ty_reserva.
...
DATA lt_agrupada TYPE STANDARD TABLE OF ty_reserva.

TYPES: BEGIN OF ty_resumen,
           aerolinea   TYPE c LENGTH 2,
           num_reservas TYPE i,
           precio_total TYPE p LENGTH 6 DECIMALS 2,
           precio_medio TYPE p LENGTH 6 DECIMALS 2,
         END OF ty_resumen.
