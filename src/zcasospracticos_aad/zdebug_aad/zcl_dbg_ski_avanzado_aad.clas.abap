CLASS zcl_dbg_ski_avanzado_AAD DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_dbg_ski_avanzado_AAD IMPLEMENTATION.
 ...
 METHOD if_oo_adt_classrun~main.
    ...
    out->write( |¿Ana es del club 7?: { lo_ana->es_del_club( iv_club_buscado = 'CLUB-GR' ) }| ).

    out->write( |Juan puede VERDE: { lo_juan->puede_bajar_pista( iv_color_pista = 'VERDE' ) }| ).
    out->write( |Sergio puede NEGRA: { lo_sergio->puede_bajar_pista( iv_color_pista = 'NEGRA' ) }| ).
    ...
    lo_grupo_2->get_monitor( )->asignar_alumno( io_alumno = lo_ana ).
    out->write( |Alumnos de Sergio tras tocar el grupo 2: { lo_grupo_2->get_monitor( )->get_alumnos_asignados( ) }| ).
    " ^ Aunque solo hemos tocado lo_grupo_2, el monitor de lo_grupo
    "   también se ve afectado. ¿Por qué?


*    DATA(lo_pista_cerrada) = NEW lcl_pista( iv_nombre = 'Pista en mantenimiento' iv_color = 'VERDE' iv_aforo_max = 10 ).
    TRY.
        DATA(lv_porcentaje) = lo_pista_verde->get_porcentaje_ocupacion( ).
        out->write( |Ocupación: { lv_porcentaje }%| ).
